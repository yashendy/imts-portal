// js/pages/index.js
// مطابق لتصميمك، مع إصلاحات:
// 1) استيراد صحيح من ../firebase.js
// 2) إنشاء وثيقة المستخدم أول مرة (بدون role)
...
/* --------- إنشاء وثيقة المستخدم إذا غير موجودة (بدون role) ---------- */
async function ensureUserDocExists(uid, email, displayName) {
  ...
  if (!snap.exists()) {
    await setDoc(ref, {
      ...
      // بدون role — التعيين يتم عبر كود التفعيل في السحابة
    });
  } else {
    // تحديث بسيط لآخر مشاهدة
...
els.form?.addEventListener("submit", async (e) => {
  ...
  try{
    if (mode === "login") {
      ...
      // onAuthStateChanged سيتكفّل بالباقي
    } else {
...
function mapAuthError(code=""){
  ...
  if (c.includes("unauthorized-domain"))
    return "الدومين غير مُصرّح به في Firebase (Authorized domains).";
