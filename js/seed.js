// js/seed.js  (one-time-per-year)
import {
  doc, setDoc, getDoc, serverTimestamp
} from "https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js";

export async function runSeed({ db, activeYearId }) {
  if (!activeYearId) throw new Error("لا توجد سنة نشطة");

  // علم تشغيل التهيئة لكل سنة (مرة واحدة)
  const flagRef = doc(db, "setupFlags", activeYearId);
  const flagSnap = await getDoc(flagRef);
  if (flagSnap.exists() && flagSnap.data().seedDone === true) {
  ...
  // مادة عربية لكل صف (اختياري)
  for (const g of grades) {
    await setDoc(doc(db, "subjects", `arabic-${g.id}`), {
      nameAr: "اللغة العربية", code:"arabic", gradeId: g.id,
      compulsory: true, active: true,
      updatedAt: serverTimestamp(), createdAt: serverTimestamp()
    }, { merge:true });
  }
  ...
  // علِّم أن السنة اتتهيأت
  await setDoc(flagRef, { seedDone: true, at: serverTimestamp(), version: 1 }, { merge: true });
